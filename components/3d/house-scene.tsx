...
import { Environment, OrbitControls, PerspectiveCamera, Html, Float, RoundedBox } from "@react-three/drei"
...
const roomDefinitions = [
  { id: "entrance", icon: Home, position: [0, 0, 0] as [number, number, number] },
  { id: "about", icon: User, position: [15, 0, 0] as [number, number, number] },
  { id: "skills", icon: Code, position: [30, 0, 0] as [number, number, number] },
  { id: "experience", icon: Briefcase, position: [45, 0, 0] as [number, number, number] },
  { id: "projects", icon: FolderOpen, position: [60, 0, 0] as [number, number, number] },
  { id: "contact", icon: MessageSquare, position: [75, 0, 0] as [number, number, number] },
]

function getRooms(content: PortfolioContent) {
  return roomDefinitions.map((room, index) => ({
    ...room,
    name: content.ui.roomNames[index],
  }))
}
              ...
              {skills.categories.map((category, i) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-3" style={{ color: skillColors[i] }}>
                    {skillIcons[i] || <Code className="w-6 h-6" />}
...
function ModernWallDisplay({ position, isDark = true }: { position: [number, number, number]; isDark?: boolean }) {
  ...
  return (
    <group position={position}>
      ...
      <Html transform position={[0, 0.1, 0.2]} scale={0.25} center>
        <div
          className="grid grid-cols-[240px_1fr] gap-8 items-center w-[1040px] rounded-[28px] p-8"
          style={{
            background: isDark
              ? "linear-gradient(135deg, rgba(15,23,42,0.96), rgba(10,10,15,0.96))"
        ...
        >
          <div className="relative">
            <div
              ...
              style={{ backgroundColor: isDark ? "rgba(20,184,166,0.25)" : "rgba(20,184,166,0.18)" }}
            />
          ...
          <div className="text-left">
            ...
            <h1 className="mb-3 text-5xl font-bold" style={{ color: colors.text }}>
              {personal.name}
            </h1>
            <p className="mb-4 text-2xl font-semibold" style={{ color: "#14b8a6" }}>
              {personal.title}
            </p>
            <p className="mb-5 text-lg leading-relaxed" style={{ color: colors.textMuted }}>
              {personal.tagline}
            </p>
              ...
              {personal.specializations.map((spec, i) => {
                ...
                return (
                  <span
                    ...
                    style={{
                      backgroundColor: isDark ? `${specColors[i]}2E` : `${specColors[i]}18`,
                      border: `2px solid ${specColors[i]}`,
            ...
            <div className="flex flex-wrap gap-3">
              <a
                ...
                className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium transition-transform hover:scale-105"
                style={{
                  backgroundColor: isDark ? "rgba(20,184,166,0.16)" : "rgba(20,184,166,0.12)",
                  color: colors.text,
              ...
              <span
                ...
                style={{
                  backgroundColor: isDark ? "rgba(129,140,248,0.16)" : "rgba(129,140,248,0.12)",
                  color: colors.text,
              ...
              <a
                ...
                style={{
                  backgroundColor: isDark ? "rgba(244,114,182,0.16)" : "rgba(244,114,182,0.12)",
                  color: colors.text,
...
function EntranceRoom({ position, isDark = true }: { position: [number, number, number]; isDark?: boolean }) {
  ...
  const socialLinks = [
    personal.github ? { href: personal.github, icon: Github, accent: "#f472b6", label: "GitHub" } : null,
    personal.linkedin ? { href: personal.linkedin, icon: Linkedin, accent: "#818cf8", label: "LinkedIn" } : null,
    { href: `mailto:${personal.email}`, icon: Mail, accent: "#14b8a6", label: "Email" },
  ].filter(Boolean) as { href: string; icon: typeof Github; accent: string; label: string }[]
      ...
      <group position={[6.7, -0.5, -1]} rotation={[0, -Math.PI / 2, 0]}>
        <Float speed={2} rotationIntensity={0.05} floatIntensity={0.2}>
          ...
          <Html transform position={[0, 0, 0.15]} scale={0.35} center>
            ...
            >
              <p className="text-lg mb-2 font-bold tracking-wider" style={{ color: "#14b8a6" }}>
                {ui.connectWithMe}
              </p>
              <p className="text-sm mb-6" style={{ color: isDark ? "#94a3b8" : "#475569" }}>
                {personal.location}
              </p>
                  ...
                  return (
                    <a
                      ...
                      style={{
                        ...
                        backgroundColor: isDark ? `${link.accent}40` : `${link.accent}24`,
                        border: `2px solid ${link.accent}`,
                      }}
                    >
                      <Icon size={36} />
                    </a>
  ...
  return (
    <Room position={position} color={isDark ? "#172554" : "#dbeafe"} isDark={isDark}>
      ...
      {experience.items.map((item, i) => (
        <FloatingFrame key={i} position={framePositions[i] || [0, -1.2, -4]} size={[3.8, 4]} isDark={isDark}>
          <div className="p-3 text-left">
...
function ContactRoom({ position, isDark = true }: { position: [number, number, number]; isDark?: boolean }) {
  const {
    content: { contact, personal, ui },
  } = usePortfolioContext()
  const socialLinks = [
    personal.github ? { href: personal.github, icon: Github, label: "GitHub" } : null,
    personal.linkedin ? { href: personal.linkedin, icon: Linkedin, label: "LinkedIn" } : null,
      ...
      <FloatingFrame position={[0, 3, -5]} size={[8, 5]} isDark={isDark}>
        <div className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-center gap-3 bg-indigo-500/20 text-indigo-200 py-3 px-4 rounded-lg">
              <Phone size={20} />
...
function Scene({ currentRoom, isDark }: { currentRoom: number; isDark: boolean }) {
  ...
  const { content } = usePortfolioContext()
  const rooms = getRooms(content)
...
export function House3D() {
  ...
  return (
    <PortfolioContext.Provider value={{ content, language, setLanguage }}>
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-10">
        <div className="flex items-center gap-2">
          <div className="bg-background/80 backdrop-blur-sm rounded-lg px-4 py-2 border border-border">
            <span className="text-primary font-semibold">{content.personal.name}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <LanguageToggle
            ...
            labels={content.ui.languages}
            title={content.ui.languageLabel}
          />
          <ThemeToggle />
        </div>
      </div>
